...
const showTab = (tabName) => {
  ...
  document
    ...
    [tabMap[tabName]].classList.add("active-tab");
...
function userEvaluate(url, expected_emotion, met_expectation) {
  fetch("/evaluate_result", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ url, expected_emotion, met_expectation }),
  })
    .then((res) => res.json())
    .then(() => {
    ...
    .catch((err) => {
      ...
      showToast("Something went wrong while saving your feedback.");
    });
}
